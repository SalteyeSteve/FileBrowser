...
<div class="embed-responsive embed-responsive-4by3"><iframe class="embed-responsive-item" src="' . $imagePath . '" ></iframe></div>
        ...
        <div class="container-fluid" id="main">
            ...
            if (is_file($root . $path)) {
                ...
                if (contains(realpath($imagePath))) {
                    echo '<img class = "show" src = "' . $imagePath . '" target="image">'; // Display image if mime type is 'images'
                    return;
                }
            ...
            <div class="container-fluid" id="text">
                <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-8">
                            <?php echo get_file_info_reader($fl);?><br><br>
                            <textarea title ="<?= basename(substr($path, 1)) ?>"></textarea>
                            </div>
                        <div class="col-lg-2"></div>
                    </div>
                    <?php
    ...
    function display ($items, $path, $root)
    {
        foreach ($items as $item) {
            ...
            if ($item->type == 'dir') {
                ...
                echo '<div id="icon"><a href="?file=' . urlencode($path) . urlencode($item->entry) . '"><img class="icons" src="icons/folder.png"><br>' . $item->entry . '</a></div>';
            } // If file
            elseif ($item->type == 'file') {

                echo '<div id="icon"><a href="?file=' . urlencode($path) . urlencode($item->entry) . '">' . get_icon(mime_content_type($item->current), realpath($item->current), $root) . '<br>' . $item->entry . '</a></div>';
            }
