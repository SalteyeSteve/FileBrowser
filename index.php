...
<html lang="en">
    ...
    <body>
        <div class="container-fluid" id="main">
            ...
            // Mime type
            $type = (mime_content_type($root . $path));
            ...
            if (is_file($root . $path)) {
                ...
                if (contains(realpath($imagePath))) {
                    ...
                    echo '<img class="mx-auto d-block" id ="show" src = "' . $imagePath . '">';
                    return;
                }
                        ...
                        <div class="container-fluid" id="text">
                            <div class="row">
                                <div class="col-lg-2 "></div>
                                    ...
                                    <textarea title ="<?= basename(substr($path, 1)) ?>"><?php readfile(substr($path, 1)); ?></textarea>
                                    </div>
                                <div class="col-lg-2"></div>
                            </div>
                        </div>
            ...
            <div class="container-fluid" id="text">
                <div class="row">
                    <div class="col-lg-3"></div>
                        <div class="col-lg-6">
                            <?php echo get_file_info_reader($fl);?><br>
                            <form action="<?php redirect($path) ?>" method="post">
                                ...
                                <textarea  id="textToSave"  name="textToSave" title="<?= basename($fl) ?>"><?php echo $text ?></textarea>
                            </form>
                        </div>
                    <div class="col-lg-3"></div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-2"></div>
                        <div class="col-lg-8 col-centered" id="label">
                            <?php echo get_file_info_reader($fl);?><br><br>
                            </div>
                        <div class="col-lg-2"></div>
                    </div>
                        ...
                        if (dirname('/?file=' . $path) !== '\\') {
                            ...
                            foreach($crumbs as $x=>$crumb) {
                                ...
                                elseif($x > 0) {
                                    ...
                                    // Add all of the breadcrumbs to one array and then erase the temporary array
            ...
            <div class="row">
                <div class="col-lg-1 col-1"></div>
                    <div class="col-lg-10 col-10" id="content">
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-10"><div class="embed-responsive embed-responsive-4by3" id="iframe"><iframe class ="embed-responsive-item" name="image"></iframe></div>
                </div>
                <div class="col-lg-2 col-1"></div>
            </div>
...
//region *FUNCTIONS*
    ...
    function display ($items, $path, $root)
    {
        foreach ($items as $item) {
            $basename =  strtok($item->entry, '.');
            // If folder
            if ($item->type == 'dir') {

                echo '<div id="icon" ><a href="?file=' . urlencode($path) . urlencode($item->entry) . '" title ="'.get_file_info(realpath($item->current)).'"><img class="icons" src="icons/folder.png"><br>' . shorten_string ($item->entry) . '</a></div>';

            } // If file
            elseif ($item->type == 'file') {

                echo '<div id="icon"><a href="?file=' . urlencode($path) . urlencode($item->entry) . '" title ="'.get_file_info(realpath($item->current)).'" >' . get_icon(mime_content_type($item->current), realpath($item->current), $root) . '<br>' . shorten_string($basename) . '</a></div>';

            }
            elseif($item->type == 'image'){

                echo '<div id="icon"><a onclick=" showDiv();" href="?file=' . urlencode($path) . urlencode($item->entry) . '" target ="image" title ="'.get_file_info(realpath($item->current)).'">' . get_icon(mime_content_type($item->current), realpath($item->current), $root) . '<br>' . shorten_string($basename) . '</a></div>';
    ...
    function get_icon ($mimeType, $file, $root)
    {
        ...
        foreach ($icon_classes as $text => $icon) {
            ...
            elseif (strpos($mimeType, $text) === 0) {
                $icon = '<img class ="icons" src="icons/' . $icon . '.png">';
                return $icon;
            }
        ...
        return '<img class="icons" src="icons/unknown.png">';
    }
...
File size:  ' . $fileSize;
    ...
    function get_sorted_entries($path){
        ...
        while (false !== ($item = readdir($dir_handle))) {
            ...
            } elseif (contains(realpath($dir))) {
                $ext = get_type(mime_content_type($dir), $item);
                $items[] = (object) array('type'=>'image','entry'=>$item, 'current'=>$dir, 'previous'=>$prev, 'extension'=>$ext);
            } else {
                $ext = get_type(mime_content_type($dir), $item);
    ...
    function contains ($image){
        $arr = explode('/', mime_content_type(realpath($image)));
